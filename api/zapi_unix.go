...
func SQLTables(statementHandle SQLHSTMT,
	...
	TableName *SQLCHAR, NameLength3 SQLSMALLINT,
	TableType *SQLCHAR, NameLength4 SQLSMALLINT) (ret SQLRETURN) {
	r := C.SQLTables(C.SQLHSTMT(statementHandle),
		(*C.SQLCHAR)(CatalogName), C.SQLSMALLINT(NameLength1),
		(*C.SQLCHAR)(SchemaName), C.SQLSMALLINT(NameLength2),
		(*C.SQLCHAR)(TableName), C.SQLSMALLINT(NameLength3),
		(*C.SQLCHAR)(TableType), C.SQLSMALLINT(NameLength4))
	return SQLRETURN(r)
}
